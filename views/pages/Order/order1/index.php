<div class="row">
				<div class="col-xxl-3 col-lg-6 col-md-6 col-12">
					<div class="box rounded-4" style="background: linear-gradient(-60deg, #1b84ff, #7fafff 35%, #1b84ff 37%);">
						<div class="box-body">
							<div class="d-flex justify-content-between align-items-center">
								<p class="m-0 fw-600 text-white">Total Orders</p>
								<div class="dropdown">
									<button class="btn btn-secondary bg-none btn-sm p-0 fs-20" data-bs-toggle="dropdown" href="#" aria-expanded="false"><span class="feather-more-vertical text-white"></span></button>
									<div class="dropdown-menu dropdown-menu-end" style="">
									  	<a class="dropdown-item" href="#">Daily</a>
								  		<a class="dropdown-item" href="#">Weekly</a>
								  		<a class="dropdown-item" href="#">Yearly</a>
									</div>
								</div>
							</div>
							<div class="mt-15">
								<p class="mb-0 mt-10 text-white">Since Last Week</p>
								<div class="d-flex justify-content-between align-items-center fw-500">
									<h1 class="my-1 fw-500 text-white">2,200</h1>
									<span class="badge badge-success-light badge-pill"><span class="feather-arrow-up"></span> + 5.5% </span>
								</div>
								<p class="mb-0 text-white">Revenue Generated: $10,500</p>
							</div>
						</div>
					</div>
				</div>
				<div class="col-xxl-3 col-lg-6 col-md-6 col-12">
					<div class="box rounded-4">
						<div class="box-body">
							<div class="d-flex justify-content-between align-items-center">
								<p class="m-0 fw-600">Completed Orders</p>
								<div class="dropdown">
									<button class="btn btn-secondary bg-none btn-sm p-0 fs-20" data-bs-toggle="dropdown" href="#" aria-expanded="false"><span class="feather-more-vertical"></span></button>
									<div class="dropdown-menu dropdown-menu-end" style="">
									  	<a class="dropdown-item" href="#">Daily</a>
								  		<a class="dropdown-item" href="#">Weekly</a>
								  		<a class="dropdown-item" href="#">Yearly</a>
									</div>
								</div>
							</div>
							<div class="mt-15">
								<p class="mb-0 mt-10">Since Last Week</p>
								<div class="d-flex justify-content-between align-items-center fw-500">
									<h1 class="my-1 fw-500">1,025</h1>
									<span class="badge badge-success-light badge-pill"><span class="feather-arrow-up"></span> + 4.1% </span>
								</div>
								<p class="mb-0">Revenue Generated: $6,500</p>
							</div>
						</div>
					</div>
				</div>
				<div class="col-xxl-3 col-lg-6 col-md-6 col-12">
					<div class="box rounded-4">
						<div class="box-body">
							<div class="d-flex justify-content-between align-items-center">
								<p class="m-0 fw-600">Pending Orders</p>
								<div class="dropdown">
									<button class="btn btn-secondary bg-none btn-sm p-0 fs-20" data-bs-toggle="dropdown" href="#" aria-expanded="false"><span class="feather-more-vertical"></span></button>
									<div class="dropdown-menu dropdown-menu-end" style="">
									  	<a class="dropdown-item" href="#">Daily</a>
								  		<a class="dropdown-item" href="#">Weekly</a>
								  		<a class="dropdown-item" href="#">Yearly</a>
									</div>
								</div>
							</div>
							<div class="mt-15">
								<p class="mb-0 mt-10">Since Last Week</p>
								<div class="d-flex justify-content-between align-items-center fw-500">
									<h1 class="my-1 fw-500">225</h1>
									<span class="badge badge-success-light badge-pill"><span class="feather-arrow-up"></span> + 3% </span>
								</div>
								<p class="mb-0">Approx Revenue: $1,500</p>
							</div>
						</div>
					</div>
				</div>
				<div class="col-xxl-3 col-lg-6 col-md-6 col-12">
					<div class="box rounded-4">
						<div class="box-body">
							<div class="d-flex justify-content-between align-items-center">
								<p class="m-0 fw-600">Cancelled Orders</p>
								<div class="dropdown">
									<button class="btn btn-secondary bg-none btn-sm p-0 fs-20" data-bs-toggle="dropdown" href="#" aria-expanded="false"><span class="feather-more-vertical"></span></button>
									<div class="dropdown-menu dropdown-menu-end" style="">
									  	<a class="dropdown-item" href="#">Daily</a>
								  		<a class="dropdown-item" href="#">Weekly</a>
								  		<a class="dropdown-item" href="#">Yearly</a>
									</div>
								</div>
							</div>
							<div class="mt-15">
								<p class="mb-0 mt-10">Since Last Week</p>
								<div class="d-flex justify-content-between align-items-center fw-500">
									<h1 class="my-1 fw-500">10</h1>
									<span class="badge badge-danger-light badge-pill"><span class="feather-arrow-down"></span> + 2.2% </span>
								</div>
								<p class="mb-0">Lost Revenue $800</p>
							</div>
						</div>
					</div>
				</div>
				<div class="col-xxl-12 col-lg-12 col-12">
				    <div class="box rounded-4 b-1">
				        <div class="box-header no-border pb-0">
				            <h3 class="fw-600 m-0">Order List</h3>
				        </div>
				        <div class="box-body">
				        	<div class="table-responsive">
					        	<table id="example-2" class="table table-hover rounded-4 overflow-hidden b-1 text-nowrap">
									<thead class="bg-light mt-5">
									    <tr>
									        <th>Order ID</th>
									        <th>Customer Name</th>
									        <th>Order Date</th>
									        <th>Product Ordered</th>
									        <th>Total Price</th>
									        <th>Payment Status</th>
									        <th>Order Status</th>
									        <th>Action</th>
									    </tr>
									</thead>
									<tbody>
									    <tr>
									        <td>#ORD121</td>
									        <td>Jhon</td>
									        <td>27/08/25</td>
									        <td>Nullacin (10)</td>
									        <td>$50</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-success-light">Completed</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD122</td>
									        <td>Emily</td>
									        <td>26/08/25</td>
									        <td>Theralief (4)</td>
									        <td>$20</td>
									        <td><span class="badge badge-pill badge-warning-light">Pending</span></td>
									        <td><span class="badge badge-pill badge-warning-light">In progress</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD123</td>
									        <td>Michael</td>
									        <td>26/08/25</td>
									        <td>CP-0004 (4)</td>
									        <td>$10</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-warning-light">Pending</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD124</td>
									        <td>Brown</td>
									        <td>26/08/25</td>
									        <td>Medicanox (4)</td>
									        <td>$20</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-success-light">Completed</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD125</td>
									        <td>Lisa</td>
									        <td>25/08/25</td>
									        <td>INV-251 (4)</td>
									        <td>$8</td>
									        <td><span class="badge badge-pill badge-danger-light">Failed</span></td>
									        <td><span class="badge badge-pill badge-danger-light">Cancelled</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD126</td>
									        <td>Paul</td>
									        <td>24/08/25</td>
									        <td>Simupril (4)</td>
									        <td>$32</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-success-light">Completed</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD127</td>
									        <td>Martinez</td>
									        <td>23/08/25</td>
									        <td>CP-0004 (4)</td>
									        <td>$12</td>
									        <td><span class="badge badge-pill badge-warning-light">Pending</span></td>
									        <td><span class="badge badge-pill badge-success-light">Completed</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD128</td>
									        <td>Wiliams</td>
									        <td>23/08/25</td>
									        <td>Simupril (5)</td>
									        <td>$5</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-warning-light">Inprogress</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD129</td>
									        <td>Moore</td>
									        <td>22/08/25</td>
									        <td>Theralief (14)</td>
									        <td>$56</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-success-light">Completed</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD130</td>
									        <td>Jhon</td>
									        <td>21/08/25</td>
									        <td>Simupril (9)</td>
									        <td>$10</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-warning-light">Inprogress</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD131</td>
									        <td>Anna Taylor</td>
									        <td>21/08/25</td>
									        <td>Medicanox (10)</td>
									        <td>$25</td>
									        <td><span class="badge badge-pill badge-success-light">Paid</span></td>
									        <td><span class="badge badge-pill badge-success-light">Completed</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									    <tr>
									        <td>#ORD132</td>
									        <td>Wiliams</td>
									        <td>20/08/25</td>
									        <td>Nullacin(5)</td>
									        <td>$36</td>
									        <td><span class="badge badge-pill badge-danger-light">Failed</span></td>
									        <td><span class="badge badge-pill badge-danger-light">Cancelled</span></td>
									        <td>
				                            	<a href="#" class="text-dark"><i class="feather-eye fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="feather-trash-2 fs-20 me-5" aria-hidden="true"></i></a>
				                            	<a href="#" class="text-dark"><i class="fa fa-share fs-20" aria-hidden="true"></i></a>
				                            </td>
									    </tr>
									</tbody>
								</table>
							</div>
				        </div>
				    </div>
				</div>
			</div>