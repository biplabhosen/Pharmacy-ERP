...
class Customer extends Model implements JsonSerializable{
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,phone,email,address,created_at,updated_at from {$tx}customers");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,phone,email,address,created_at,updated_at from {$tx}customers $criteria limit $top,$perpage");
		$data=[];
		while($customer=$result->fetch_object()){
			$data[]=$customer;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,phone,email,address,created_at,updated_at from {$tx}customers where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		while($customer=$result->fetch_object()){
			...
			$html.="<tr><td>$customer->id</td><td>$customer->name</td><td>$customer->phone</td><td>$customer->email</td><td>$customer->address</td><td>$customer->created_at</td><td>$customer->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
