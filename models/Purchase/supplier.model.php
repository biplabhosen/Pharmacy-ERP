...
class Supplier extends Model implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}suppliers(name,phone,email,address,created_at,updated_at)values('$this->name','$this->phone','$this->email','$this->address','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}suppliers set name='$this->name',phone='$this->phone',email='$this->email',address='$this->address',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,phone,email,address,created_at,updated_at from {$tx}suppliers");
		$data=[];
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,phone,email,address,created_at,updated_at from {$tx}suppliers $criteria limit $top,$perpage");
		$data=[];
		while($supplier=$result->fetch_object()){
			$data[]=$supplier;
		}
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,phone,email,address,created_at,updated_at from {$tx}suppliers where id='$id'");
...
";
	...
	static function html_select($name="cmbSupplier"){
		...
		$result =$db->query("select id,name,phone,email,address from {$tx}suppliers");
		while($supplier=$result->fetch_object()){
			$html.="<option data-phone='$supplier->phone' data-email='$supplier->email' data-address='$supplier->address' value ='$supplier->id'>$supplier->name</option>";
		}
		$html.="</select>";
		return $html;
	}
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Phone</th><th>Email</th><th>Address</th><th>Created At</th><th>Updated At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Phone</th><th>Email</th><th>Address</th><th>Created At</th><th>Updated At</th></tr>";
		}
		while($supplier=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$supplier->id</td><td>$supplier->name</td><td>$supplier->phone</td><td>$supplier->email</td><td>$supplier->address</td><td>$supplier->created_at</td><td>$supplier->updated_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
